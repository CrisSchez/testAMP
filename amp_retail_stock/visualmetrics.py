...
agg_metrics = metrics_df.dropna(subset=["metrics.mse"]).sort_values('startTimeStampMs')
sns.barplot(x=list(range(1,len(agg_metrics)+1)), y="metrics.mse", color="grey", data=agg_metrics)
plt.savefig('accuracyEvolution.png')


latest_aggregate_metric = metrics_df.dropna(subset=["metrics.mse"]).sort_values('startTimeStampMs')[-1:]["metrics.mse"]
...
a=np.sqrt(a)
valores=np.linspace(0,6,5)
import bisect
flecha=bisect.bisect_left(valores, a)

gauge(labels=['GOOD','CARE','NOT GOOD','LOW','CRITICAL'], colors=['#007A00','#0063BF','#FFCC00','#F18517','#ED1C24'],    arrow=flecha, title='smse ='+str(np.round(a,2))) 
